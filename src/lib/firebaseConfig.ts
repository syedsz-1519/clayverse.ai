 ...
 * 
 * NOTE: Firebase is optional. If env variables are not configured,
...
if (isConfigValid) {
  try {
    ...
    if (import.meta.env.DEV && import.meta.env.VITE_USE_FIREBASE_EMULATOR === 'true') {
      try {
        // Auth Emulator
        if (auth && !auth.currentUser && auth.app.automaticDataCollectionEnabled) {
    ...
    console.warn('⚠️ Firebase is not available. App will work with local storage only.');
  }
} else {
  console.warn('⚠️ Firebase not initialized - environment variables missing. App will work with local storage only.');
}

/**
 * Export configured Firebase services
 * All exports can be null if Firebase is not configured
 */
export { app, auth, firestore, storage, analytics, isConfigValid };

/**
 * Utility function to log analytics events
 * Safe to call even if analytics is null
 */
